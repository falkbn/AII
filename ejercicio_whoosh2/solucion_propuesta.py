...
def apartado_b(dirindex):
    ...
    try:
        with ix.searcher() as searcher:
            ...
            for r in results:
                print("Fecha: " + r['fecha'].strftime('%d-%m-%Y'), "   RTTE: " + r['remitente'],
                      "   DESTINARIOS: " + r['destinatarios'], "   ASUNTO: " + r['asunto'])
    except:
...
def add_doc(writer, path, docname):
    try:
        ...
        writer.add_document(remitente=rte, destinatarios=dtos, fecha=dat, asunto=ast, contenido=ctdo,
                            nombrefichero=docname)
